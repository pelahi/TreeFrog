.. TreeFrog documentation master file, created by
   sphinx-quickstart on Mon Jul 31 10:13:40 2017.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.
   **TreeFrog** is a C++ halo merger tree builder using MPI and OpenMP APIs.
   It comes several associated analysis tools in python, example configuration files
   and analysis python scripts (and sample jupyter notebooks).

Welcome to **TreeFrog**'s documentation!
#############################################

**TreeFrog** is a C++ halo merger tree builder using MPI and OpenMP APIs.
The repository also contains several associated analysis tools in python, example configuration files
and analysis python scripts (and sample jupyter notebooks).


.. toctree::
   :maxdepth: 2
   :numbered:
   :titlesonly:
   :glob:
   :hidden:
   :caption: Contents:
   getting.rst
   usage.rst
   output.rst



Sections
========
* :ref:`getting` : How to compile the code
* :ref:`usage` : How to run the code
* :ref:`output` : How to use output data
* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
