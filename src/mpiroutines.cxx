...
/// now if using mpi then can broadcast the sets to construct final unique set
...
map<IDTYPE, IDTYPE> MPIGatherIDs(Options &opt, vector<IDTYPE> &idvec)
{
    ...
    unsigned long long commsize, oldsize;
    ...
    for (auto i=0;i<NProcs;i++) sendtask[i]=recvtask[i]=-1;
    //task zero is explicitly never a send task
    for (auto i=NProcs-1;i>=1;i-=2) {sendtask[i]=i-1;sendset.push_back(i);}
    for (auto i=NProcs-2;i>=0;i-=2) {recvtask[i]=i+1;recvset.push_back(i);}
    ...
    do {
        ...
        if (!(sendtask[ThisTask]==-1 && recvtask[ThisTask]==-1)) {
            ...
            if (sendtask[ThisTask]>=0 && recvtask[ThisTask]==-1) {
                ...
                {
                    for (auto i=0;i<chunksize;i++) idarray[i]=idvec[offset+i];
        ...
        //reset tasks
        for (auto j=0;j<nsendtasks;j++) sendtask[sendset[j]]=-1;
        for (auto j=0;j<nrecvtasks;j++) recvtask[recvset[j]]=-1;
        //only tasks that were receiving before are part of the new send/recv set
        for (auto j=0;j<nrecvtasks-1;j+=2) sendtask[recvset[j]]=recvset[j+1];
        for (auto j=1;j<nrecvtasks;j+=2) recvtask[recvset[j]]=recvset[j-1];
        sendset.clear();
        recvset.clear();
        for (auto j=NProcs-1;j>=0;j--) if (sendtask[j]>=0) sendset.push_back(j);
        for (auto j=NProcs-1;j>=0;j--) if (recvtask[j]>=0) recvset.push_back(j);
    ...
    for (auto ichunk=0;ichunk<nchunks;ichunk++)
    {
        if (ThisTask==0) for (auto i=chunksize-1;i>=0;i--) idarray[i]=idvec[vecsize-1-i];
        MPI_Bcast(idarray,chunksize, MPI_Id_type, 0, MPI_COMM_WORLD);
        for (auto i=chunksize-1;i>=0;i--) idmap.insert(pair<IDTYPE, IDTYPE>(idarray[i],(IDTYPE)(commsize-1-i-offset)));
...
void MPILoadBalanceSnapshots(Options &opt)
{
    ...
    if (ThisTask==0) {
        ...
        unsigned long long *numinfo=new unsigned long long[opt.numsnapshots];
        unsigned long long *mpi_sum=new unsigned long long[NProcs]();
        unsigned long long totpart;
...
void MPISendProgenitorsUsingDescendants(int recvtask, int isnap,HaloTreeData *&pht, DescendantDataProgenBased **&pprogendescen, ProgenitorData **&pprogen)
{
    ...
    unsigned long long *tothaloindex;
    ...
    int *nremoval;
    unsigned long long *removalnoffset;
    unsigned long long *removaltothaloindex;
    ...
    if (totitems>0) {
        ...
        noffset=new unsigned long long[pht[isnap].numhalos];
        tothaloindex=new unsigned long long[totitems];
        ...
        removalnoffset=new unsigned long long[pht[isnap].numhalos];
        removaltothaloindex=new unsigned long long[removaltotitems];
        ...
        MPI_Send(&noffset[0],pht[isnap].numhalos, MPI_LONG_LONG, recvtask, isnap*NProcs*NProcs+ThisTask+3*NProcs, MPI_COMM_WORLD);
        MPI_Send(&tothaloindex[0],totitems, MPI_LONG_LONG, recvtask, isnap*NProcs*NProcs+ThisTask+4*NProcs, MPI_COMM_WORLD);
        ...
        MPI_Send(&removalnoffset[0],pht[isnap].numhalos, MPI_LONG_LONG, recvtask, isnap*NProcs*NProcs*NProcs+ThisTask+3*NProcs, MPI_COMM_WORLD);
        MPI_Send(&removaltothaloindex[0],removaltotitems, MPI_LONG_LONG, recvtask, isnap*NProcs*NProcs*NProcs+ThisTask+4*NProcs, MPI_COMM_WORLD);
...
void MPIRecvProgenitorsUsingDescendants(int sendtask, int isnap, HaloTreeData *&pht, DescendantDataProgenBased **&pprogendescen, ProgenitorData **&pprogen)
{
    ...
    if (totitems>0) {
        ...
        MPI_Recv(&noffset[0],pht[isnap].numhalos, MPI_LONG_LONG, sendtask, isnap*NProcs*NProcs+sendtask+3*NProcs, MPI_COMM_WORLD,&status);
        MPI_Recv(&tothaloindex[0],totitems, MPI_LONG_LONG, sendtask, isnap*NProcs*NProcs+sendtask+4*NProcs, MPI_COMM_WORLD,&status);
        ...
        MPI_Recv(&removalnoffset[0],pht[isnap].numhalos, MPI_LONG_LONG, sendtask, isnap*NProcs*NProcs*NProcs+sendtask+3*NProcs, MPI_COMM_WORLD,&status);
        MPI_Recv(&removaltothaloindex[0],removaltotitems, MPI_LONG_LONG, sendtask, isnap*NProcs*NProcs*NProcs+sendtask+4*NProcs, MPI_COMM_WORLD,&status);
